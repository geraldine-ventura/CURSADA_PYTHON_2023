...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.btn_pedir_clave = customtkinter.CTkButton(
            master=self, text="Ingresar", command=self.btn_pedir_clave_on_click
        )
        self.btn_pedir_clave.grid(row=2, pady=20, columnspan=2, sticky="nsew")

    def btn_pedir_clave_on_click(self):
        clave = prompt(title="clave", prompt="Ingrese una contraseña: ").lower()

        while clave != "utn750":
            clave = prompt(
                title="clave", prompt="clave incorrecta, ingrese nuevamente: "
            )
