...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        importe_float=self.txt_descuento.get()
        importe_str= str(importe_float)

        descuento_float=self.txt_descuento.get()
        descuento_str= str(descuento_float)

        descuento_aplicado= importe_float * (descuento_float/100)
        importe_actualizado= importe_float + descuento_aplicado
        importe_actualizado_str=str(importe_actualizado)

        mensaje="El importe actualizado aplicando el descuento del {0}, es: {1}".format(descuento_str,importe_actualizado_str)
